...
// Modal klik luar tutup
window.addEventListener("click", (e) => {
  if(e.target === taskModal) taskModal.style.display = "none";
});

saveTaskBtn.addEventListener("click", addSubject);
...
function addSubject() {
  ...
  if(!name) return alert("Isi nama mata pelajaran dulu!");
  subjects.push({ name, deadline, emoji, color, indicators: [] });
  saveData();
  render();
...
function addIndicator(index) {
  subjects[index].indicators.push({ text:"", deadline:"", done:false });
  saveData();
  render();
}
...
function toggleIndicator(sIndex,iIndex){
  subjects[sIndex].indicators[iIndex].done = !subjects[sIndex].indicators[iIndex].done;
  saveData();
  render();
}
  ...
  subjects.forEach((sub,sIndex)=>{
    ...
    card.innerHTML=`
      <div style="display:flex;justify-content:space-between;align-items:center;">
        ...
        <button onclick="deleteSubject(${sIndex})">🗑</button>
      </div>

      <div class="progress-bar"><div style="width:${percent}%"></div></div>

      ${percent>0?`<div class="mission-status">🚀 MISSION ${percent}%</div>`:''}
      ...
      ${sub.indicators.map((ind,iIndex)=>{
        ...
        <div class="indicator-card ${ind.done?'done':''} ${statusClass}">
          <div class="indicator-top">
            <input type="checkbox" ${ind.done?'checked':''} onchange="toggleIndicator(${sIndex},${iIndex})">
            <input type="text" placeholder="Tulis indikator..." value="${ind.text}" oninput="updateIndicatorText(${sIndex},${iIndex},this.value)">
          </div>
          <div class="indicator-bottom">
            <input type="date" value="${ind.deadline}" onchange="updateIndicatorDeadline(${sIndex},${iIndex},this.value)">
